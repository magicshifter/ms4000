  ...
  void flash(int start, int end) {
    ...
    for (byte idx = start; !stopLoop; idx += delta) {
      msSystem.msLEDs.setLED(idx,
                             ...
                             // 255, 255, 255,
                             // safety orange
                             255, 121, 0, msGlobals.ggBrightness);
...
public:
  ...
  uint8_t colors[6][3] = {{255, 0, 0}, {0, 255, 160}, {255, 121, 2},
                          {0, 2, 1},   {1, 2, 0},     {2, 1, 0}};
  ...
  void simpleBouncingBall(int colorIndex) {
    ...
    for (byte idx = 0; idx < MAX_LEDS; idx++) {
      ...
      // scale *= 10;
      // msSystem.msLEDs.setLED(idx, (msGlobals.ggCurrentFrame & 1) ?
      // msGlobals.ggBrightness*scale : 0, (msGlobals.ggCurrentFrame & 2) ?
      // msGlobals.ggBrightness*scale : 0, (msGlobals.ggCurrentFrame & 4) ?
      // msGlobals.ggBrightness*scale : 0, msGlobals.gs);
