 ...
 *
 * Utilizes miby for MIDI Byte Parsing, from Neil Johnson
 * <http://www.njohnson.co.uk>
 *
 * NOTE: camelBack for types and functions, snake_case for vars,
 * SCREAMING_SNAKE_CASE for constant definitions
 *
 */
...
private:
  ...
  const uint8_t note_LUT[7][7] = {{0, 1, 2, 2, 3, 4, 5}, {0, 1, 1, 2, 3, 4, 4},
                                  {0, 0, 1, 2, 3, 3, 4}, {0, 1, 2, 3, 3, 4, 5},
                                  {0, 1, 2, 2, 3, 4, 4}, {0, 1, 1, 2, 3, 3, 4},
                                  {0, 0, 1, 2, 2, 3, 4}};
...
#define NUM_ARP_PATTERNS 8
  const ArpPatternT arp_patterns[NUM_ARP_PATTERNS][4] = {
      // arp_patterns 4/4
      {{0, BEAT1, EIGHTH},
       {2, BEAT2, EIGHTH},
       {0, BEAT3, EIGHTH},
...
public:
  ...
  uint8_t arp_frame =
      0; // internal beat, which is 1/16th of the actual beat (see arp_bpm)
  ...
  // ---------------------------------------------------------------------------
  // ARPEGGIATOR Process a frame of the Arpeggiator
  void arpFrame() {
    ...
    if (arp_frame == 0 &&
      ...
      while (event_idx < 4) {
        // if you don't use getNextNaturalNote in arpNoteOn, use
        // getNextNaturalNote here instead
        uint8_t note = getNextNaturalNoteAdd(
            new_note, arp_patterns[new_pattern][event_idx].note_offset);
        // on
  ...
  void arpPlayNote(uint8_t noteNumber, uint8_t on_off) {
    ...
    msSystem.slogln("ARP: Note: " + String(noteNumber) +
                    " vel: " + String(current_velocity) + " ON/OFF: " +
                    String((on_off ? MIDI_NOTE_ON : MIDI_NOTE_OFF) +
                           curr_midiview.midi_channel));
  ...
  // arpSendMIDI should be called after queueing up all messages and associated
  // data for those messages
  void arpSendMIDI() {
#ifdef CONFIG_ENABLE_MIDI_SERIAL
    while (arp_fifo_in_index !=
           arp_fifo_out_index) { // send whatever is in the buffer
...
public:
  ...
  MIDIArpeggiatorMode() { modeName = "Arpi"; }
  ...
  void incPattern() {
    ...
    msSystem.msLEDs.setLED(_arp.new_pattern, 0, 100, 100,
                           msGlobals.ggBrightness);
  ...
  bool step() {
    ...
    if (!((nowT - _arp.last_arp_frame_time) < _arp.arp_beat_duration)) {
      ...
      // msSystem.slogln("MIDI: nowT - last_arp_frame_time:" + String((nowT -
      // _arp.last_arp_frame_time)));
    }
  ...
  void MIDI_Program_Change(byte channel, byte program) {
    msSystem.slogln(modeName + " MIDI PC - channel: " + String(channel) +
  ...
  void MIDI_Control_Change(byte channel, byte cc1, byte cc2) {
    msSystem.slogln(modeName + " MIDI CC - channel: " + String(channel) +
                    " cc1: " + String(cc1) + " cc2: " + String(cc2));
  ...
  void MIDI_Note_On(byte channel, byte note, byte velocity) {
    msSystem.slogln(modeName + " MIDI Note On - channel: " + String(channel) +
                    " note: " + String(note) + " vel: " + String(velocity));
  ...
  void MIDI_Note_Off(byte status, byte note, byte velocity) {
    msSystem.slogln(modeName + " MIDI Note Off - status: " + String(status) +
                    " note: " + String(note) + " vel: " + String(velocity));
