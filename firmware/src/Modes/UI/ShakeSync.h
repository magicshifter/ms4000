...
/*
POVShakeSyncDummyMode provides a dummy implementation if u have to satisfy the
ShakeSyncInterface For example if you want to refactor something
...
public:
  ...
  int getFrameIndex() { return -1; }

  // returns true if POV shake is actve
  bool update(float g) { return true; }
};
...
class POVShakeSync {
  ...
  // last time it took to go from acceleration minimum to maximum/ from max to
  // min
  int min2maxDelta, max2minDelta;
  int min2maxFrames, max2minFrames;

  // we use these to track the timestamp of the current min resp. max, might
  // change since we are not sure we have found it yet
  ...
  // at each moment we might be either searching for the next min (then
  // searchMin == true) or max then searchMin == false
  bool searchMin = false;

  // used to make sure the events only fire once the first time their condition
  // is met
  ...
  // are we in a shaking position that has a frame to display? Is frameIndex
  // valid?
  bool isFrameIndexActive = false;

public:
  POVShakeSync(void) {}
  void setFrames(int w) { frames = w; }
  ...
  bool update(float g) {
    ...
    // workaround for strage behaviour of shake code, final fix needs moar
    // debugging ;) sometimes frameIndex remains static but isActive is not set
    // to false by above code workaround just detects the situation and resets
    // isActive
    ...
    // msSystem.msLEDs.fillLEDs(0, 0, 0); // only enable this for debugging
    // shaking
    ...
    if (searchMin) {
      ...
      // could be timeout of condition above (g < activeMin.g) then we dont need
      // hysteresis
      if (g > (activeMin.g + hysteresis)) {
        ...
        frameStartTime = lastMin.micros + min2maxDelta / 2 -
                         ((((float)min2maxDelta) / min2maxFrames) * frames) / 2;
        //((((float) min2maxFrames) / min2maxDelta) * frames) / 2;
        // end previous frame since it wont fit anyway (we are allready moving
        // in other dir)
        isFrameIndexActive = false;
      ...
      // else if (!firedPredictedZero && (msGlobals.ggCurrentMicros >
      // lastMax.micros + min2maxDelta/2)) // ikes else if (!firedPredictedZero
      // && (msGlobals.ggCurrentMicros > lastMax.micros + max2minDelta/2)) //
      // correct
        ...
        // fillLEDs(5, 0, 0);
                 ...
                 msGlobals.ggCurrentMicros >= lastMax.micros + max2minDelta) {
      ...
      if (g < (activeMax.g - hysteresis)) {
        ...
        frameStartTime = lastMax.micros + max2minDelta / 2 -
      ...
      // else if (!firedPredictedZero && (msGlobals.ggCurrentMicros >
      // lastMin.micros + max2minDelta/2)) // ikes else if (!firedPredictedZero
      // && (msGlobals.ggCurrentMicros > lastMin.micros + min2maxDelta/2)) // ok
      else if (!firedPredictedZero &&
               msGlobals.ggCurrentMicros > frameStartTime) {
        frameIndex = 0;
        isFrameIndexActive = true;
    ...
    // msSystem.msLEDs.updateLEDs(); // // only enable this for debugging
    // shaking
