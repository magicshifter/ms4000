...
bool parseAPInfoFromServerArgs(APAuth &apInfo) {
  ...
  for (int i = 0; i < msSystem.msESPServer.args(); i++) {
    ...
    if (strcmp(msSystem.msESPServer.argName(i).c_str(), "ssid") == 0) {
      ...
      l_safeStrncpy(apInfo.password, msSystem.msESPServer.arg(i).c_str(),
...
bool parseSysLogInfoFromServerArgs(ServerConfig &sysLogHost) {
  ...
  for (int i = 0; i < msSystem.msESPServer.args(); i++) {
    ...
    if (strcmp(msSystem.msESPServer.argName(i).c_str(), "host") == 0) {
      l_safeStrncpy(sysLogHost.hostname, msSystem.msESPServer.arg(i).c_str(),
...
void handleGetSettings(void) {
  ...
  response += "<li><a href=\"/settings/server\">server settings "
              "(/settings/server)</a></li>";
  response += "<li><a href=\"/settings/ap\">accesspoint settings "
              "(/settings/ap)</a></li>";
  ...
  response += "<li><a href=\"/settings/wifi/list\">list of saved wifis "
              "(/settings/wifi/list)</a></li>";
  response += "<li><a href=\"/settings/wifi/preferred\">preferred wifi "
              "(/settings/wifi/preferred)</a></li>";
...
void handlePOSTServerSettings(void) {
  ...
  if (msSystem.msESPServer.args() >= 2) {
    ...
    for (int i = 0; i < msSystem.msESPServer.args(); i++) {
      ...
      if (strcmp(msSystem.msESPServer.argName(i).c_str(), "host") == 0) {
        l_safeStrncpy(config.hostname, msSystem.msESPServer.arg(i).c_str(),
...
bool ms3kModeLightNameFunc(pb_istream_t *stream, const pb_field_t *field,
                           void **arg) {
...
void handleGETProtoBufferBase64(void) {
  ...
  base64length =
      base64_encode(base64Output, (char *)pbufOutput, stream.bytes_written);
...
void handlePOSTpbufBase64(void) {
  ...
  if (msSystem.msESPServer.args() == 0) {
    msSystem.msESPServer.send(500, "text/plain", "argument missing!");
  } else if (msSystem.msESPServer.args() >= 1) {
    ...
    l_safeStrncpy(decoderInput, msSystem.msESPServer.arg(0).c_str(),
                  PROTOBUF_LEN);
    ...
    decodedDataLen =
        base64_decode((char *)decoderOutput, decoderInput, encodedInputStrLen);
    ...
    decodeStatus = pb_decode(&stream, MS4_fields,
                             &msGlobals.pbuf); // TODO: !J! msGlobals.pbuf??
...
void handleSetMode(void) {
  ...
  if (msSystem.msESPServer.args() == 1) {
    ...
    for (int i = 0; i < msSystem.msESPServer.args(); i++) {
      ...
      int newMode = atoi(msSystem.msESPServer.arg(i).c_str());
...
void handleSetUISettings(void) {
  ...
  if (msSystem.msESPServer.args() >= 1) {
    ...
    for (int i = 0; i < msSystem.msESPServer.args(); i++) {
      ...
      if (strcmp(msSystem.msESPServer.argName(i).c_str(), "timeoutHighPower") ==
          0) {
        config.timeoutHighPower = atoi(msSystem.msESPServer.arg(i).c_str());
      } else if (strcmp(msSystem.msESPServer.argName(i).c_str(),
                        "timeoutLowPower") == 0) {
