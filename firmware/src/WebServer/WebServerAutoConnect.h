...
void WifiOnStationModeDisconnected(
    const WiFiEventStationModeDisconnected WifiEvent) {
  msSystem.slogln("WiFi STA disconnect");
}

void WifiOnStationModeAuthModeChanged(
    const WiFiEventStationModeAuthModeChanged WifiEvent) {
...
void WifiOnStationModeDHCPTimeout() { msSystem.slogln("WiFi DHCP Timeout"); }

void WifiOnSoftAPModeStationConnected(
    const WiFiEventSoftAPModeStationConnected WifiEvent) {
  msSystem.slogln("WiFi Soft AP STA connect");
}

void WifiOnSoftAPModeStationDisconnected(
    const WiFiEventSoftAPModeStationDisconnected WifiEvent) {
...
void WiFiEvent(WiFiEvent_t event) {
  ...
  case WIFI_EVENT_STAMODE_GOT_IP:
    msSystem.slogln("WiFi connected - IP address: " +
...
bool TrySoftAP(struct APAuth &apInfo) {
  ...
  //  WiFi.softAPConfig(IPAddress(10,20,30,40), IPAddress(10,1,1,1),
    ...
    if (LittleFS.exists(msSystem.Settings.apListConfigPath)) {
      ...
      if (numNets == 0) {
        ...
        for (int i = 0; i < numNets; ++i) {
          ...
          msSystem.slog(String("  ") + String(i + 1) + String(": ") +
      ...
      while (msSystem.Settings.getNextAP(&msGlobals.ggAPConfig.apInfo.auth)) {
        for (int i = 0; i < numNets; i++) {
          if (strcmp(WiFi.SSID(i).c_str(),
            ...
            if (TryConnect(msGlobals.ggAPConfig.apInfo.auth,
                           CONNECTION_TIMEOUT)) {
