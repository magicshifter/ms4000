...
void handleNotFound() {
  ...
  // if we don't have a route for the request, but do have a file, serve the
  // file
  if (LittleFS.exists(fName + 1) || LittleFS.exists(gzFName.c_str())) {
    ...
    String redirectToHostname =
        "http://" + msSystem.Settings.getUniqueSystemName() + ".local/";
    ...
    String message = "<html>\
      ...
      <body>\
        <h1>Upload of " +
                     String(msGlobals.ggUploadFileName) +
                     " done</h1><a href=\"/list?dir=\">list</a><br><a "
                     "href=\"\">index.html</a></body></html>";
...
void handleFileList() {
  ...
  while (true) {
    ...
    */
    ...
    output += "<p><a href=\"download?file=" + name + "\">" + name + "</a> " +
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    l_safeStrncpy(msGlobals.ggUploadFileName, (char *)upload.filename.c_str(),
                  MAX_FILENAME_LENGTH); //.c_str();
    ...
    if (LittleFS.exists(msGlobals.ggUploadFileName)) {
      ...
      msSystem.slogln(String(LittleFS.remove(msGlobals.ggUploadFileName)));
    }
    // !J! re-use file..
    ...
    while (--cnt >= 0) {
      ...
      msGlobals.ggUploadFile = LittleFS.open(msGlobals.ggUploadFileName, "w");
      if (msGlobals.ggUploadFile)
        break;
    }
    ...
    // *)upload.filename.c_str()); msGlobals.ggUploadFile =
    // SD.open(upload.filename.c_str(), FILE_WRITE);
