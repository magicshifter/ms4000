  ...
  do {
    ...
    do {
      ...
      if (bitMask & currentByte) {
        // msLEDs.setLED(ledIdx, r, g, b, 255); // this could be more efficient
...
void PlotBitmapColumn24Bit(const MSBitmap *bitmap, uint16_t absColumn,
                           uint8_t startLed, byte *frameDest) {
  ...
  // ReadBytes(offSet, msGlobals.ggRGBLEDBuf + 3*startLed, nrOfBytes); // never
  // make startLed too big or it will corrupt mem
...
void PlotBitmapColumn(const MSBitmap *bitmap, uint8_t frame, uint8_t column,
                      uint8_t startLed, byte *frameDest) {
  uint8_t frameIdx = frame - bitmap->header.firstChar;
  if (frameIdx <= bitmap->header.maxFrame) {
    uint16_t absColumn =
        ((uint16_t)frameIdx) * ((uint16_t)bitmap->header.frameWidth) + column;
...
void PlotText(const MSBitmap *bitmap, const char *text, uint16_t column,
              uint8_t startLed, byte *frameDest) {
  ...
  void getFrameData(int frameIdx, byte *frameDest) {
    ...
    for (int i = 0; i < txtCount; i++) {
      ...
      if (idx >= 0 && idx < txtSizes[i]) {
        PlotText(&txtFonts[i], txtCollection[i], idx, txtPositions[i].y,
                 frameDest);
      }
    }
  };
...
public:
  ...
  MagicShifterImage() { height = width = 0; }
  MagicShifterImage(const char *fileName) { LoadFile(fileName); }

  ~MagicShifterImage() { close(); }
  ...
  static bool LoadBitmapFile(const char *filename, MSBitmap *bitmap) {
    ...
    bitmap->bmFile.read((uint8_t *)(&(bitmap->header)), sizeof(MSBitmapHeader));
