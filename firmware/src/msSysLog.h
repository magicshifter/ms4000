...
public:
  ...
  void connect_wifi() {
    ...
    if (msGlobals.ggDebugSerial) {
      ...
      Serial.println("syslog: WiFi: " + String((WiFi.status() == WL_CONNECTED)
                                                   ? "connected"
                                                   : "not connected"));
  ...
  void setup(char *syslogServerIPStr) {
    ...
    Serial.println("syslog: user-configured host: " +
                   String(syslogServerIPStr));
    Serial.println("syslog: configured host is IP: " +
                   syslogServerIP.toString());
  ...
  void sendSysLogMsg(String aMsg) {
    // String newMsg = " 009.local <45>MAGICSHIFTER:" + aMsg;	// !J! todo: fix
    // level/service?
