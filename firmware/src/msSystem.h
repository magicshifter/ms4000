...
class MagicShifterSystem {
  ...
  public:
    ...
    bool getAPConfig(struct APAuth *config) {
      ...
      msSystem.slogln("webserver: not configured, using unique name: " +
                      String(config->ssid));
    ...
    void deleteAP(char *ssid) {
      ...
      if (inFile) {
        ...
        while (inFile.read((uint8_t *)&inAPAuth, sizeof(APAuth)) ==
               sizeof(APAuth)) {
      ...
      if (apListIndex >= 0) {
        ...
        do {
          if (smAPListFile.read((uint8_t *)apInfo, requiredBytes) ==
              requiredBytes) {
            apListIndex++;
            if (!msSystem.msEEPROMs.memcmpByte((byte *)apInfo, 0,
                                               requiredBytes))
              return true;
          } else {
            return false;
...
public:
  ...
  void slog(String msg) { msSysLog.sendSysLogMsg(msg); };
  void slogln(String msg) { msSysLog.sendSysLogMsg(msg); };
  ...
  //  void slog(bool b) { if (b) slog(String("true")); else
  //  slog(String("false")); }
  ...
  //  void slogln(bool b) { if (b) slogln(String("true")); else
  //  slogln(String("false")); }
  ...
  void slogSysInfo() {
    ...
    slogln("Flash real size (method b): " +
    ...
    if (ESP.getFlashChipSize() != ESP.getFlashChipRealSize()) {
      slogln(String(
          "WARNING: configured flash size does not match real flash size!"));
    }
...
#warning                                                                       \
    "BUILD configuraion will FORCE FORMAT on execution - disabled, come and un-comment it..."
//		int fmtRet;
...
//		slogln("lfs format result = " + String(fmtRet));
  ...
  void feedbackAnimation(int mode) {
    ...
    msLEDs.setLED(msGlobals.ui.currentMode, 128, 128, 128,
  ...
  void infinite_swipe() {
    while (1) {
      ...
      for (byte idx = 0; idx < MAX_LEDS; idx++) {
        msLEDs.setLED(idx, (idx & 1) ? 255 : 0, (idx & 2) ? 255 : 0,
                      (idx & 4) ? 255 : 0, msGlobals.ggBrightness);
        msLEDs.updateLEDs();
        delay(30);
      }
      for (byte idx = 0; idx < MAX_LEDS; idx++) {
...
#define BRIGHTNESS_CONTROL_TIME (850 * 1000)
  ...
  uint8_t BrightnessLevels[16] = {1,  2,  3,  4,  5,  6,  7,  8,
                                  10, 12, 14, 16, 18, 22, 26, 31};
      ...
      while (skip) {
        ...
        if (newV == 0) {
          ...
          for (uint8_t i = 0; i <= 15; i++) {
            ...
            // if (dB <= 16) dB = 16;
            if (newV >= lBr)
              msLEDs.setLED(15 - i, BRIGHTNESS_UI_LEVEL, BRIGHTNESS_UI_LEVEL,
...
#define LIPO_DISPLAY_UPPER_LIMIT_V 4.5
  ...
  void showBatteryStatus(bool shouldFadeIn) {
    ...
    for (int i = 0; i >= 0 && i <= 15; i++) {
      float iV = LIPO_DISPLAY_LOWER_LIMIT_V +
                 (LIPO_DISPLAY_UPPER_LIMIT_V - LIPO_DISPLAY_LOWER_LIMIT_V) *
                     (i / 16.0);

      if (batLevel > iV) {
        int red, green;
        if (iV > LIPO_DISPLAY_RED_LIMIT_V) {
          green = 255 * (iV - LIPO_DISPLAY_RED_LIMIT_V) /
                  (LIPO_DISPLAY_UPPER_LIMIT_V - LIPO_DISPLAY_RED_LIMIT_V);
        } else
          green = 0;

        if (iV < LIPO_DISPLAY_ORANGE_LIMIT_V) {
          red = 255 * (LIPO_DISPLAY_ORANGE_LIMIT_V - iV) /
                (LIPO_DISPLAY_ORANGE_LIMIT_V - LIPO_DISPLAY_LOWER_LIMIT_V);
        } else
          red = 0;
        msLEDs.setLED(15 - i, red, green, 0, gs);

        // msLEDs.setLED(i, 0, iV > LIPO_DISPLAY_RED_LIMIT_V  ? 150 : 0, iV <
        // LIPO_DISPLAY_ORANGE_LIMIT_V ? 150 : 0, gs);
      }
...
#ifdef CONFIG_ENABLE_MIDI_SERIAL
#warning                                                                       \
    "SERIAL MIDI has been enabled - Serial I/O at 31250 - serial logging disabled (use wlan)"
    Serial.begin(31250);
#else
#warning "SERIAL MIDI is disabled - syslog will use serial logging"
  ...
  void calculateBatteryLevel() {
    batteryVoltage =
        calculateVoltage(msGlobals.ggLastADValue, msGlobals.batVoltCalibration);
  ...
  void powerDownStep() {
    ...
    if (msGlobals.ui.timeoutHighPower != 0) {
      ...
      if (msPowerCountDown + msGlobals.ui.timeoutHighPower <
          msGlobals.ggCurrentMillis) {
        powerDown();
      }
    }

    //
    if ((lowBatteryMillis != 0) && (msGlobals.ui.timeoutLowPower != 0) &&
        (lowBatteryMillis + (10 * 1000) <
         msGlobals.ggCurrentMillis)) { // 10 seconds
      // note! unsigned values!
      if (msPowerCountDown + msGlobals.ui.timeoutLowPower <
          msGlobals.ggCurrentMillis) {
        powerDown();
      }
    }
  ...
  // internal system loop - handles the system UI, basic buttons,
  // mode-switching, etc.
  void step() {
  ...
  void enableLongClicks(bool enable) { msButtons.msLongClickOK = enable; }
