...
APPLEMIDI_CREATE_INSTANCE(
    WiFiUDP, AppleMIDI, "MS3000-midi",
    DEFAULT_CONTROL_PORT); // see definition in AppleMidi_Defs.h
...
void _RTP_MIDI_Note_On(byte channel, byte note, byte velocity) {
  msSystem.slog("MIDI(rtp): Note channel:" + String(channel) +
                " noteOn:" + String(note) + " vel.:" + String(velocity));
}

void _RTP_MIDI_Note_Off(byte channel, byte note, byte velocity) {
  msSystem.slog("MIDI(rtp): Note channel:" + String(channel) +
                " noteOff:" + String(note) + " vel.:" + String(velocity));
}

void _RTP_MIDI_ReceiveAfterTouchPoly(byte channel, byte note, byte velocity) {
  msSystem.slog("MIDI(rtp): channel:" + String(channel) + " atouch poly note:" +
                String(note) + " vel.:" + String(velocity));
};
...
void _RTP_MIDI_ReceiveClock() { msSystem.slogln("MIDI(rtp): Clock"); };

void _RTP_MIDI_ReceiveStart() { msSystem.slogln("MIDI(rtp): START"); };

void _RTP_MIDI_ReceiveContinue() { msSystem.slogln("MIDI(rtp): CONTINUE"); };

void _RTP_MIDI_ReceiveStop() { msSystem.slogln("MIDI(rtp): STOP"); };

void _RTP_MIDI_ReceiveActiveSensing() {
  msSystem.slogln("MIDI(rtp): ACTIVE SENSE");
};

void _RTP_MIDI_ReceiveReset() { msSystem.slogln("MIDI(rtp): RESET"); };
...
// {}; void OnContentInvitationAccepted(void* sender,
