...
uint16_t SERIAL_MIDI_Get(uint8_t *data, uint16_t length) {
  ...
  while ((Serial.available()) &&
         ((length > 0) && (data[count] = Serial.read()) != -1)) {
    count++;
    length--;
  }
  if (count != 0)
...
void SERIAL_MIDI_init() { miby_init(&miby_serial, NULL); }

void SERIAL_MIDI_loop() {
  // this is the per-frame midi byte
  if (SERIAL_MIDI_Get(&midi_in_byte, 1) != 0) {
    // msSystem.msLEDs.setLED(LED_NOTE_EVENT, 100, 100, 100,
...
void SERIAL_MIDI_Control_Change(miby_this_t a_miby) {
  ...
  msGlobals.ggModeList[msGlobals.ui.currentMode]->MIDI_Control_Change(
      MIBY_CHAN(a_miby), MIBY_ARG0(a_miby), MIBY_ARG1(a_miby));
}
...
void SERIAL_MIDI_Note_Off(miby_this_t a_miby) {
  ...
  msGlobals.ggModeList[msGlobals.ui.currentMode]->MIDI_Note_Off(
      MIBY_STATUSBYTE(a_miby), MIBY_ARG0(a_miby), MIBY_ARG1(a_miby));
}
